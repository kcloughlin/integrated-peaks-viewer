...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 15 12:22:52 2025
...
from qtpy.QtWidgets import QApplication, QMainWindow, QWidget, QFileDialog, QMessageBox, QPushButton, QGridLayout, QVBoxLayout, QLabel, QLineEdit, QComboBox, QTextEdit, QShortcut
...
class View(QWidget):
    ...
    def get_hklmnp(self):
        return [int(i.text()) for i in [self.h_line,self.k_line,self.l_line,self.m_line,self.n_line,self.p_line]]
...
class Presenter:
    ...
    def select_config(self):
        ...
        if self.view.yaml_line.text() == '':
            filename, ok = QFileDialog().getOpenFileName(self.view,'Select file','/SNS/','Garnet config files (*.yaml)')
        else:
            file_folder = '/'.join(self.view.yaml_line.text().split('/')[:-1])
            filename, ok = QFileDialog().getOpenFileName(self.view,'Select file',file_folder,'Garnet config files (*.yaml)')
        self.clear_all()
        if filename:
            path = Path(filename)
            self.view.yaml_line.setText(str(path))
            self.view.text_out = str(path) + '\n'
            self.view.output.setPlainText(self.view.text_out)
            
            self.init_nxs(str(path))
        self.initialized = False
                    
        
    def init_nxs(self,path):
        self.integrate_path = path.rstrip('yaml').rstrip('.') + "_integration/"
        nexus_paths = Path(self.integrate_path).rglob('*(max)=????.nxs')
        nexus_paths = [str(i).split('/')[-1] for i in nexus_paths]
        
        if len(nexus_paths) > 0:
            self.view.nxs_combo.addItems(nexus_paths)
        else:
            self.view.text_out += f'\nNo integration files found for {path}'
            self.view.output.setPlainText(self.view.text_out)
    ...
    def update_hkl(self):
        ...
        plots_paths = self.model.list_plots(self.integrate_path,self.view.get_nxs(),self.view.get_hklmnp())
        
        if plots_paths != -1:
            plots_sorted = [[i.split('#')[-1].strip('.png'),i] for i in plots_paths]
            plots_sorted.sort()
            plots_paths = [i[1] for i in plots_sorted]
        
        if plots_paths == -1:
            self.view.text_out += f'\nNo plots found for (h,k,l) = ({h},{k},{l}) with (m,n,p) = ({m},{n},{p})  \n'
            self.view.run_combo.clear()
            self.clear_plot()
        else:
            self.view.text_out += f'\n{len(plots_paths)} plots found for (h,k,l) = ({h},{k},{l}) with (m,n,p) = ({m},{n},{p})\n  '
...
class Model:
    ...
    def list_plots(self,int_path,nxs,hklmnp):
        ...
        plots_folder = '/'+'/'.join(int_path.split('/')) + nxs.rstrip('nxs').rstrip('.') + '_plots/'
        plot_path = Path(plots_folder).rglob('*({},{},{})_({},{},{})/*.png'.format(*hklmnp))
        plots_path = [str(i) for i in plot_path]
    ...
    def plot(self,plot,run,integrate_path,nexus):
        ...
        filename = integrate_path + nexus.rstrip('nxs').rstrip('.') + '_plots/' + run
...
class IntegratedPeaksViewer(QMainWindow):
    ...
    def __init__(self,parent=None):
        ...
        self.setGeometry(50,50,1250,1250)#723,804)
